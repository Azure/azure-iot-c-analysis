...
int main()
{
    ...
    else
    {
        ...
        if ((handle = Prov_Device_Create(g_global_prov_uri, id_scope, g_prov_transport)) == NULL)
        {
        ...
        {
            Prov_Device_SetOption(handle, "logtrace", &g_trace_on);
            if (trusted_cert != NULL)
            {
                Prov_Device_SetOption(handle, OPTION_TRUSTED_CERT, trusted_cert);
            }
            Prov_Device_SetOption(handle, PROV_REGISTRATION_ID, "manual-reg-id");

            if (Prov_Device_Register_Device(handle, register_device_callback, &user_ctx, registation_status_callback, &user_ctx) != PROV_DEVICE_RESULT_OK)
            {
